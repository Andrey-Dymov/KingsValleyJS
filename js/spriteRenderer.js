...
import {
  ...
  PS_NONE, MOVE_NORMAL, MOVE_KNIFE, MOVE_PICKAX,
...
// Цвет прозрачности в спрайтах MSX (magenta / тёмно-серый)
...
function getProcessedFrame(img, frameIndex) {
  const key = `${img.src || img.width}_${frameIndex}`;
...
/**
 * Рисует спрайт из изображения (горизонтальная полоса кадров)
 * Магента и тёмно-серый заменяются на прозрачность
 * Flip: translate(x+16,y) + scale(-1,1) + draw(0,0) — спрайт остаётся на месте
 */
export function drawSprite(ctx, img, frameIndex, x, y, flip = false) {
  const src = getProcessedFrame(img, frameIndex);
  ctx.save();
  if (flip) {
...
export function renderJewel(ctx, entity, tilesImg) {
  const cycle = Math.floor(entity.frame / FRAME_WAIT) % 3;
  const tiles = [JEWEL_TILE, JEWEL_EFFECT_UP_TILE, JEWEL_EFFECT_LEFT_TILE];
  const tileIdx = tiles[cycle];
  drawTile(ctx, tilesImg, tileIdx, entity.x, entity.y);
}
